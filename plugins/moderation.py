...
class Moderation(commands.Cog):
    ...
    @cog_ext.cog_slash(name="rule",
        options=[create_option(name="Rule", option_type=3, description="Rule to get", required=True, choices=[create_choice(name=f"Rule {rule}", value=rule) for rule in range(len(rules))])]
    )
